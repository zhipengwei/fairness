...
#include "/home/wzp/workspace/ns-allinone-3.26/ns-3.26/scratch/configOnOffBreakConnection.h"
#include <vector>
...
{
  NS_LOG_FUNCTION (this);
  // Check whether the socket is empty or not, this should create a new socket.
  if (!m_socket)
    {
      NS_LOG_FUNCTION (Simulator::Now() .GetSeconds() << ("Create a new socket."));
      m_socket = Socket::CreateSocket (GetNode (), m_tid);
      if (Inet6SocketAddress::IsMatchingType (m_peer))
        {
          m_socket->Bind6 ();
...
void MyOnOffApplication::StopSending ()
{
  ...
  m_socket->Close();
  m_socket = 0;
  NS_LOG_FUNCTION (Simulator::Now().GetSeconds() << ("Assign 0 to a socket pointer."));
  ScheduleStartEvent ();
}
  ...
  if (m_maxBytes == 0 || m_totBytes < m_maxBytes)
    {
      ...
      NS_LOG_LOGIC ("m_maxBytes = " << m_maxBytes);
      NS_LOG_LOGIC ("m_totBytes = " << m_totBytes);
...
void
MyOnOffApplication::CwndChange (std::string context, uint32_t oldCwnd, uint32_t newCwnd)
{
  NS_LOG_UNCOND ("CongestionWindow" << context << "\t" << Simulator::Now ().GetSeconds () << "\t" << newCwnd);
}
...
// Create OnOff applications to send TCP to the hub, one on each spoke node.
  ...
  //    onOffHelper.SetAttribute ("Remote", sinkAddress);
	ApplicationVector[i] = CreateObject<MyOnOffApplication> ();
        ApplicationVector[i]->SetMaxBytes (1000000);
	ApplicationVector[i]->SetAttribute ("Remote", sinkAddress);
        ApplicationVector[i]->SetAttribute ("OnTime", StringValue (ConvertToStringValue(EXPERIMENT_SENDER_ONTIME_CONSTANT, "constant")));
